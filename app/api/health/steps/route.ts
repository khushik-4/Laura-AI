...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(
      `https://api.fitbit.com/1/user/-/activities/steps/date/${sevenDaysAgo.toISOString().split('T')[0]}/${today.toISOString().split('T')[0]}.json`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to fetch steps data" }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data["activities-steps"].map((activity: any) => ({
